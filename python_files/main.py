...
class Bot(commands.Bot):
    ...
    def __init__(self):
        ...
        super().__init__(
            token=data.get("TWITCH_TOKEN"),
            prefix="!",
            initial_channels=[data.get("TWITCH_CHANNEL")],
        )
    ...
    async def event_message(self, message):
        ...
        if message.author.name == data.get("TWITCH_CHANNEL"):
        ...
        if Bot.done[0] == True:
            count = 0
            while (new_message in Bot.previous_responses) and (
                count <= len(Bot.previous_responses)
            ):
                new_message = Bot.current_messages[
                    randint(0, len(Bot.current_messages) - 1)
                ]
                count += 1
